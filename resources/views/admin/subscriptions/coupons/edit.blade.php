    ...
    <div class="content-wrapper">
        ...
        <div class="row">
            <div class="col-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Coupon Update form </h4>
                        <form class="forms-sample" method="POST" action="{{ route('admin.coupon.update', $coupon->id)}}">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="exampleInputEmail3">Code</label>
                                <input type="text" class="form-control" id="exampleInputEmail3"
                                    placeholder="Code for the Coupon" name="code" value="{{ old('code', $coupon->code) }}">
                            </div>
                            <div class="form-group">
                                <label for="exampleTextarea1">Description</label>
                                <textarea class="form-control" id="exampleTextarea1" cols="30" rows="10" name="description"
                                    placeholder="Coupon description">{{ old('description', $coupon->description) }}</textarea>
                            </div>
                            <div class="form-group">
                                <label for="exampleInputEmail3">Discount</label>
                                <div style="display: flex; gap: 15px; align-items: center;">
                                    <input type="number" class="form-control" id="exampleInputEmail3"
                                        placeholder="Discount" name="discount" value="{{ old('discount', $coupon->discount) }}">
